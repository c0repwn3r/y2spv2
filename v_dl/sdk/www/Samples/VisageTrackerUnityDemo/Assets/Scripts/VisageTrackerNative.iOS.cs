...
public static partial class VisageTrackerNative
{
    ...
    */
     ...
     */
    [DllImport (dllName)]
    public static extern bool _openCamera(int orientation, int device, int width, int height, int mirrored);
 ...
 */
    ...
    */
    [DllImport(dllName, CharSet = CharSet.Ansi)]
    static extern int _recognize(short[] descriptor, int n, [In, Out] String[] names, int names_cap, float[] similarities);
    ...
    public static int _recognizeWrapper(short[] descriptor, int n, List<string> names, float[] similarities)
    {
        ...
        String[] sbArray = new String[n];
        for (int i = 0; i < n; ++i)
        {
            sbArray[i] = new string(' ',SB_CAPACITY);
        }
        //
